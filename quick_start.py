...
def show_results(results_dir, enhanced=False):
    ...
    enhanced_results_files = list(Path(results_dir).glob("enhanced_benchmark_results_*.json"))
    ...
    try:
        ...
        if enhanced and 'instructlab_metrics' in results:
            ...
            print(f"   📏 Avg Response Length: {quality_metrics.get('average_response_length', 0):.1f}")
            print(f"   🌈 Response Diversity: {quality_metrics.get('response_diversity', 0):.4f}")
            ...
            if synthetic_impact:
                print(f"   🧪 Synthetic Data Ratio: {synthetic_impact.get('synthetic_ratio', 0):.2%}")
                estimated_improvement = synthetic_impact.get('estimated_improvement', {})
                print(f"   📊 Data Diversity Score: {estimated_improvement.get('data_diversity_score', 0):.4f}")
                print(f"   📈 Coverage Enhancement: {estimated_improvement.get('coverage_enhancement', 0):.4f}")
                print(f"   🎯 Quality Boost: {estimated_improvement.get('quality_boost', 0):.4f}")
            
            # Knowledge injection
            knowledge_injection = instructlab_metrics.get('knowledge_injection', {})
            if knowledge_injection:
                print(f"   🧠 Knowledge Retention: {knowledge_injection.get('knowledge_retention_score', 0):.4f}")
                print(f"   🎯 Domain Accuracy: {knowledge_injection.get('domain_accuracy', 0):.4f}")
                print(f"   ✅ Factual Consistency: {knowledge_injection.get('factual_consistency', 0):.4f}")
            ...
            if isinstance(results.get('semantic_similarity'), (int, float)):
                ...
                print(f"🧠 Semantic Similarity: {results.get('semantic_similarity', 'N/A')}")
            ...
            if isinstance(results.get('avg_inference_time_ms'), (int, float)):
                ...
                print(f"⚡ Avg Inference Time: {results.get('avg_inference_time_ms', 'N/A')} ms")
...
def run_enhanced_pipeline(use_instructlab=True, synthetic_samples=100, domain="custom"):
    ...
    if use_instructlab:
        ...
        try:
            ...
            if not run_command([sys.executable, "-m", "pip", "install", "-r", "requirements-instructlab.txt"], 
                             "Installing InstructLab dependencies", check=False):
...
def run_enhanced_training(synthetic_samples=100, domain="custom"):
    ...
    command = [
        sys.executable, "scripts/enhanced_training.py",
        "--config", config_file,
        "--synthetic-samples", str(synthetic_samples),
        "--domain", domain,
        "--experiment-name", f"enhanced-qlora-{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ]
...
def run_enhanced_benchmark(model_path):
    ...
    command = [
        sys.executable, "scripts/enhanced_benchmark.py",
        "--model", model_path,
        "--test-data", test_data,
        "--output", results_dir,
    ...
    if run_command(command, "Enhanced benchmarking with InstructLab metrics", check=False):
        return results_dir
    return None

def main():
    """Main function with command line argument support"""
    parser = argparse.ArgumentParser(description="QLORAX Enhanced Quick Start with InstructLab")
    parser.add_argument("--mode", choices=["standard", "enhanced"], default="enhanced",
...
def run_demo_mode():
    ...
    try:
        ...
        result = run_command([sys.executable, "scripts/instructlab_integration.py"], 
                           "InstructLab integration demo", check=False)
        
        if result:
            print("✅ InstructLab integration demo completed successfully!")
        else:
            print("⚠️  InstructLab integration demo had issues (this is normal for demo mode)")
